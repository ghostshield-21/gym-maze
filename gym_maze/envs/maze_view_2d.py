...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None, 
                 ...
                 has_loops=False, num_portals=0, enable_render=True, num_picks=0):
        ...
        if maze_file_path is None:
            self.__maze = Maze(maze_size=maze_size, has_loops=has_loops, num_portals=num_portals, num_picks=num_picks)
        else:
            ...
            self.__maze = Maze(maze_cells=Maze.load_maze(maze_file_path), num_portals=0, has_loops=has_loops, num_picks=num_picks)
    ...
    def __draw_robot(self, colour=(255, 0, 0), transparency=255):
    ...
    def __draw_picks(self, transparency=160):

        if self.__enable_render is False:
            return
        
        colour_range = np.linspace(0, 255, len(self.maze.picks), dtype=int)
        colour_i = 0
        for pick in self.maze.picks:
            colour = ((100 - colour_range[colour_i])% 255, colour_range[colour_i], 0)
            colour_i += 1
            self.__colour_cell(pick, colour=colour, transparency=transparency)
...
class Maze:
    ...
    def __init__(self, maze_cells=None, maze_size=(10,10), has_loops=True, num_portals=0, num_picks=0,):
    ...
    def __set_random_portals(self, num_portal_sets, set_size=1):
        # find some random cells to break
        ...
        for i in range(num_portal_sets):
            # sample the set_size number of cell
    ...
    def __set_random_picks(self):
        # limit the maximum number of pick  to the number of cells available.
        if self.num_picks>0:
            max_picks= int(self.MAZE_W * self.MAZE_H / 4)
            num_picks = min(max_picks, int(self.num_picks))

            # creating coordinates for comparison 
            result = np.where(self.maze_cells == 0)
            ZeroCoordinates= list(zip(result[0], result[1]))
            result = np.where(self.maze_cells != 0)
            nonZeroCoordinates= list(zip(result[0], result[1]))

            # creating all possible picking coordiance 
            pick_coords_all = []
            for coord in nonZeroCoordinates:
                for direction in ['N', 'W', 'E', 'S']:
                    ...
                    if (new_direction in ZeroCoordinates):
                        pick_coords_all.append(coord)

            if len(pick_coords_all) > num_picks:
                self.__picks=[]
                self.__loaded_picks=[]
    ...
    def load_pick(self, cell):
        if cell in self.__picks:
            self.__picks.remove(cell)
            self.__loaded_picks.append(cell)

    def get_picks_all(self):
        return self.__picks

    @property
    def loaded_picks(self):
        return (self.__loaded_picks)

    def get_loaded_picks(self):
        return self.loaded_picks
...
if __name__ == "__main__":
    ...
    maze = MazeView2D(screen_size= (500, 500), maze_file_path='D:/notebooks/MMAI 845/gym-maze/gym_maze/envs/maze_samples/maze_warehouse.npy', maze_size=(10,10), num_portals=0, has_loops=False , num_picks=4)
    maze.update()
